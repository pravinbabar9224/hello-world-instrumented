...
node {
  // Check if there's a previous deployment and get the image version so we can rollback if needed
  try {
    prevImageTag = sh(
  ...
  // Quay access is registered as a secret, inspect the secret and extract
  ...
  stage ('Quay.io Login') {
    ...
    sh("docker login -u=\"${quayUserName}\" -p=\"${quayPassword}\" quay.io")
  }
